...
def do_and_form(expressions, env):
    ...
    while expression_index.rest:
        if is_scheme_false(scheme_eval(expression_index.first, env)):
            return False
        expression_index = expression_index.rest
    return scheme_eval(expression_index.first, env)
    # END PROBLEM 12
...
def do_or_form(expressions, env):
    ...
    expression_index = expressions
    while expression_index.rest: 
        if is_scheme_true(scheme_eval(expression_index.first, env)):
            return scheme_eval(expression_index.first, env)
        expression_index = expression_index.rest
    return scheme_eval(expression_index.first, env)
    # END PROBLEM 12
